...
# Prefer GNU date (gdate on macOS) for %N sub-second precision.
if [ -n "$(date +%N 2>/dev/null | tr -dc 0-9)" ] && [ "$(date +%N 2>/dev/null)" != "N" ]; then
	_UUIDV7_DATE=date
...
export _uuidv7_last_time_ms=$("$_UUIDV7_DATE" +%s%3N)
export _uuidv7_current_time_ms="$_uuidv7_last_time_ms"
_uuidv7_sequencer() {
  # Get current timestamp in milliseconds
  _uuidv7_current_time_ms=$("$_UUIDV7_DATE" +%s%3N)
  ...
  _uuidv7_last_time_ms=$_uuidv7_current_time_ms
